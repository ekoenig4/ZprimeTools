...
void ZprimeJetsClass2017::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
    ...
    double leadingEleRecoSF_corr=1.0;
	...
	if (true) {
		...
		if(elelist.size() == 2){
			...
			if (elePt->at(lepindex_leading) < 500) {
			  ...
			  leadingEleEffSF_corr=h_eleIDSF->GetBinContent(h_eleIDSF->GetXaxis()->FindBin(eleEta->at(lepindex_leading)),h_eleIDSF->GetYaxis()->FindBin(elePt->at(lepindex_leading)));
			  // std::cout<<"eleEffSF_corr =  "<< eleEffSF_corr<<std::endl;
			}
			else if (elePt->at(lepindex_leading) >= 500) {
			  leadingEleRecoSF_corr=h_eleRecoSF_highpt->GetBinContent(h_eleRecoSF_highpt->GetXaxis()->FindBin(2.49),h_eleRecoSF_highpt->GetYaxis()->FindBin(499));
			  // std::cout<<"eleRecoSF_corr =  "<< eleRecoSF_corr<<std::endl;
			  leadingEleEffSF_corr=h_eleIDSF->GetBinContent(h_eleIDSF->GetXaxis()->FindBin(2.49),h_eleIDSF->GetYaxis()->FindBin(499));
			  // std::cout<<"eleEffSF_corr =  "<< eleEffSF_corr<<std::endl;
			}
			if (elePt->at(lepindex_subleading) < 500) {
			  subleadingEleRecoSF_corr=h_eleRecoSF_highpt->GetBinContent(h_eleRecoSF_highpt->GetXaxis()->FindBin(eleEta->at(lepindex_subleading)),h_eleRecoSF_highpt->GetYaxis()->FindBin(elePt->at(lepindex_subleading)));
			  // std::cout<<"eleRecoSF_corr =  "<< eleRecoSF_corr<<std::endl;
			  subleadingEleEffSF_corr=h_eleIDSF->GetBinContent(h_eleIDSF->GetXaxis()->FindBin(eleEta->at(lepindex_subleading)),h_eleIDSF->GetYaxis()->FindBin(elePt->at(lepindex_subleading)));
			  // std::cout<<"eleEffSF_corr =  "<< eleEffSF_corr<<std::endl;
			}
			else if (elePt->at(lepindex_subleading) >= 500) {
			  subleadingEleRecoSF_corr=h_eleRecoSF_highpt->GetBinContent(h_eleRecoSF_highpt->GetXaxis()->FindBin(2.49),h_eleRecoSF_highpt->GetYaxis()->FindBin(499));
			  // std::cout<<"eleRecoSF_corr =  "<< eleRecoSF_corr<<std::endl;
...
vector<int> ZprimeJetsClass2017::JetVetoDecision(int leading_lep_index, int subleading_lep_index) {
  ...
  for(int i = 0; i < nJet; i++){
    ...
    if(deltar_leading>0.4 && deltar_subleading>0.4 && jetPt->at(i) >30.0 && tightJetID && loosePUID)
