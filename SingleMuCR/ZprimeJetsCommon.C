...
void ZprimeJetsClass::fillHistos(int histoNumber,double event_weight) {
  ...
  if(jetCand.size()>0){
    ...
    h_ChPtFrac[histoNumber]->Fill(chargedPtFrac,event_weight);
...
void ZprimeJetsClass::getPt123Frac() {
  ...
  chargedPtFrac = HadronPtFirst3[1]/HadronPt[1];
  ChNemPtFrac = (HadronPtFirst3[1]+HadronPtFirst3[2])/(HadronPt[1]+HadronPt[2]);
  for (int i = 0; i < 3; i++) hadronTotPtFrac[i] = HadronPtFirst3[i]/jetPt->at(jetCand[0]);
}
...
void ZprimeJetsClass::AllPFCand(vector<int> jetCand,vector<int> PFCandidates) {
  ...
  Pt123Fraction=Pt123=j1Mass=chargedPtFrac=ChNemPtFrac=0.0;
  for (int i = 0; i < 3; i++) hadronTotPtFrac[i] = 0.;
