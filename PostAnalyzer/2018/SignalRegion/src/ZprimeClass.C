	...
	if(jetCand.size()>0) {
	  ...
	  if (pfMET > recoilCut) {
	    ...
	    if(metcut < metRatioCut) {
	      ...
	      bool noLeptonID = (electron_veto(jetindex,eleLoosePtCut) && muon_veto(jetindex,muLoosePtCut) &&
				 photon_veto(jetindex,phoLoosePtCut) && tau_veto(jetindex,tauLoosePtCut));
	      if( noLeptonID )  {
