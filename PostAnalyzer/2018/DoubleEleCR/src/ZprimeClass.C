...
void ZprimeClass::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
	...
	if(jetCand.size()>0) {
	  ...
	  vector<int> elelist_leading = electron_tightID(jetindex,eleTightPtCut);
	  ...
	  if( CRSelection(elelist_leading,elelist_subleading) ) {
		...
		bool muVeto = muon_veto(jetindex,lepindex_leading,lepindex_subleading,muLoosePtCut);
...
void ZprimeClass::BookHistos(const char* outputFilename) {
  ...
  cutflow = new Cutflow( {"Total Events","metFilters","Trigger","GoodJet","CRSelection","leptoMetCut","dileptonMassCut","LeptonVeto","caloMETCut","B-JetVeto","DeltaPhiCut","eleHEMVeto"});
