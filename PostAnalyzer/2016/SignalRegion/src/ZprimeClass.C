	...
	if(jetCand.size()>0) {
	  ...
	  if (pfMET > recoilCut) { // MET > 250
	    ...
	    if(metcut < metRatioCut) { // metcut < 0.5
	      ...
	      bool noLeptonID = electron_veto(jetindex,eleLoosePtCut) && muon_veto(jetindex,muLoosePtCut) && photon_veto(jetindex,phoLoosePtCut);
	      if( noLeptonID )  {
...
void ZprimeClass::BookHistos(const char* outputFilename) {
  ...
  cutflow = new Cutflow({"Total Events","metFilters","Trigger","GoodJet","MetCut","caloMETCut","LeptonIDs","B-JetVeto","DeltaPhiCut"});
