...
void ZprimeYear::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  int nTotal = 0;
  float nTotalEvents,nFilters, nHLT, nCRSelection, nMET200, ndilepton, nNoElectrons, nMETcut,nbtagVeto, nDphiJetMET,nJetSelection,eleHEMVeto;
...
vector<int> ZprimeYear::electron_veto_looseID(int jet_index, int leading, int subleading, float elePtCut) {
...
vector<int> ZprimeYear::tau_veto_looseID(int jet_index,int leading,int subleading,float tauPtCut) {
  ...
  for (int itau : tmpcands ) {
    float dR_leading = deltaR(tau_Eta->at(itau),tau_Phi->at(itau),muEta->at(leading),muPhi->at(leading));
    float dR_subleading = deltaR(tau_Eta->at(itau),tau_Phi->at(itau),muEta->at(subleading),muPhi->at(subleading));
    if ( dR_leading > leptondRCut && dR_subleading > leptondRCut )
