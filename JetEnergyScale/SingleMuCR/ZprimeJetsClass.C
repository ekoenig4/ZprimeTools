...
void ZprimeJetsClass::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  int nTotal = 0;
  double nTotalEvents,nFilters, nHLT, nCRSelection, nMET200, lepMET_MT160, nNoElectrons, nMETcut,nbtagVeto, nDphiJetMET,nJetSelection,eleHEMVeto,nTotalEvents_wPU;
  nTotalEvents = nFilters = nHLT = nCRSelection = nMET200 = lepMET_MT160 = nNoElectrons = nMETcut = nDphiJetMET = nbtagVeto = nJetSelection = eleHEMVeto = nTotalEvents_wPU = 0;
  vector<int> jetveto;
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
	...
	if (HLTMet>>7&1 == 1 || HLTMet>>8&1 == 1 || HLTMet>>10&1 == 1 || !sample.isData) {
	  ...
	  if (jetCand.size() > 0) {
		...
		if (elelist.size() == 0) {
		  if (unc == 0) nNoElectrons+=event_weight;
		  ...
		  if (unc == 0) h_lepMET_MT->Fill(lepMET_MT);
		
		  if (lepMET_MT < 160) {
		    if (unc == 0) lepMET_MT160+=event_weight;
		    if (unc == 0) fillHistos(7,event_weight);
		    if (unc == 0) h_metcut->Fill(metcut,event_weight);
			...
			if (unc == 0) h_dphimin->Fill(minDPhiJetMET_first4,event_weight);
		      
			if (dPhiJetMETcut(jetveto)) {
			  if (unc == 0) nDphiJetMET+=event_weight;
			  if (unc == 0) fillHistos(10,event_weight);
			
			  if (getEleHEMVeto(40)) {
			    if (unc == 0) eleHEMVeto+=event_weight;
			    if (unc == 0) fillHistos(11,event_weight); //norm
			    if (unc == 1)  fillHistos(12,event_weight); //up
			    if (unc == -1) fillHistos(13,event_weight);//down
			  }
			}
		      }
		    }
...
void ZprimeJetsClass::BookHistos(const char* outputFilename) {
  ...
  h_cutflow = new TH1D("h_cutflow","h_cutflow",12,0,1);h_cutflow->Sumw2();
